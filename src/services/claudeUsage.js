...
function build({ live, manual, rate } = {}) {
  ...
  const plan = (manual.plan && String(manual.plan)) || 'Max';
  ...
  const NOTES = {
    live: 'Live · claude.ai',
    manual: 'Manual entry',
    unavailable: 'Connected — usage not readable, enter manually',
    setup: 'Not connected — log in or enter manually',
  };
...
function metricRow(key, label, used, resets, color) {
  ...
  return {
    ...
    leftText: `${left}% left`, // what the dock shows on the right
